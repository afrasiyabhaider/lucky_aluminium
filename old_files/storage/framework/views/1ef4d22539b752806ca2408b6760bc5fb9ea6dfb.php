...
<?php if(!empty($contact->opening_balance) && $contact->opening_balance != '0.00'): ?>
    <?php if( $contact->type == 'customer' || $contact->type == 'both'): ?>
        <strong><?php echo app('translator')->getFromJson('lang_v1.total_due'); ?></strong>
        <p class="text-muted">
        <span class="display_currency" data-currency_symbol="true">
        <?php echo e(($contact->opening_balance - $contact->opening_balance_paid) + ($contact->total_invoice - $contact->invoice_received), false); ?></span>
        </p>
    <?php endif; ?>
<?php endif; ?>
