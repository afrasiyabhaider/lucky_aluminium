		...
		<div class="small-box bg-gray">
            <div class="inner">
            	...
            	<table class="table no-margin no-border table-slim">
            		<tr><th><?php echo app('translator')->getFromJson('restaurant.placed_at'); ?></th><td><?php echo e(\Carbon::createFromTimestamp(strtotime($order->created_at))->format(session('business.date_format')), false); ?> <?php echo e(\Carbon::createFromTimestamp(strtotime($order->created_at))->format('h:i A'), false); ?></td></tr>
            		<tr><th><?php echo app('translator')->getFromJson('restaurant.order_status'); ?></th><td><span class="label <?php if($order->res_order_status == 'cooked' ): ?> bg-red <?php elseif($order->res_order_status == 'served'): ?> bg-green <?php else: ?> bg-light-blue <?php endif; ?>"><?php echo app('translator')->getFromJson('restaurant.order_statuses.' . $order->res_order_status); ?> </span></td></tr>
