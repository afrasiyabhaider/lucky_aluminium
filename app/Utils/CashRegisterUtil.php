...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterTransactionDetails($user_id, $open_time, $close_time, $is_types_of_service_enabled = false)
    {
        ...
        $expense_details = Transaction::where('transactions.created_by', $user_id)
                ...
                ->first();
        $deposits = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transaction_date', [$open_time, $close_time])
                ->where('transactions.type', 'deposit')
                ->where('transactions.status', 'received')
                ->where('transactions.payment_status', 'paid')
                ->select(
                    DB::raw('SUM(final_total) as total_deposits')
                )
                ->first();
        $withdrawals = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transaction_date', [$open_time, $close_time])
                ->where('transactions.type', 'withdraw')
                ->where('transactions.status', 'received')
                ->where('transactions.payment_status', 'paid')
                ->select(
                    DB::raw('SUM(final_total) as total_withdrawals')
                )
                ->first();
